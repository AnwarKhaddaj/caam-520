...
void wavefront520(double* data, int nx, int ny, int Nx, int Ny) {
    ...
    for(int w = 0; w < waves; w++) {
        counter = 0;
        //order the blocks and store in arrays
    ...
    //calculate number of full "cycles" of thread size
    ...
    //leftover portion in fully parallel region
    if (leftover != 0) {
        #pragma omp parallel for
...
//helper function to calculate how many elements in a wave
int elements(int wave, int I, int J, int waves) {
    if (wave <= min(I, J)) 
        return wave + 1;
